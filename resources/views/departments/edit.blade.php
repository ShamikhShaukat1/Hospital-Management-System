...
@section('content')
    <div class="max-w-xl mx-auto">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h3 class="text-lg font-bold text-slate-800 mb-4 pb-3 border-b">Edit Department: {{ $department->name }}</h3>
            ...
            <form method="POST" action="{{ route('departments.update', $department) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-600 focus:outline-none">{{ old('description', $department->description) }}</textarea>
                    @error('description')
                        <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="pt-4 border-t flex justify-end gap-2">
                    <a href="{{ route('departments.index') }}"
                        class="px-4 py-2 border rounded-lg text-sm font-semibold text-slate-600">Cancel</a>
                    <button type="submit"
