...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('departments.create') }}" class="px-4 py-2 bg-teal-700 hover:bg-teal-800 text-white rounded-lg text-sm font-semibold shadow-sm flex items-center gap-2">
        ...
        @forelse($departments as $dept)
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="px-2 py-0.5 text-xs font-semibold rounded-full {{ $dept->status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-600' }}">
                        {{ ucfirst($dept->status) }}
                    </span>
                </div>

                <h4 class="font-bold text-slate-800 text-base mb-1">{{ $dept->name }}</h4>
                <p class="text-xs text-slate-500 mb-4 line-clamp-2">{{ $dept->description ?? 'No description provided.' }}</p>
                ...
                <div class="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                    ...
                    <a href="{{ route('departments.delete', $dept) }}" class="text-slate-400 hover:text-rose-600">
